# CONTRACT — `GET /diff` (working-tree changes + git context)
...
- `Collect` fills the same `git` object from the status read it is already
  ...
  one.
- `BridgeVersion` 6 records the capability. The app does **not** gate on that
  number: an older bridge simply omits `git`, which decodes to "not a repo" and
  the action hides itself — the endpoint answering is what unlocks it.
