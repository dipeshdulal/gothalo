...
| GET  | `/diff` | — (query: `pane`, `context?`) | `{branch, git{}, files[]}` | an **agent** pane's working-tree changes — branch + git context (default branch, remote, ahead/behind, dirty, changed count, unfinished merge/rebase) + one unified diff per file. `?context=1` returns the `git` object alone and skips the diff (see [`CONTRACT-diff.md`](CONTRACT-diff.md)) |
...
`performer` says **who carries it out**, and it is the one field here that is not
cosmetic. `app` suggestions the app performs itself. An `agent` suggestion is
sent to the agent in the pane as `params.prompt`, and the client **must show that
prompt and let it be edited before sending**. An absent or unrecognised value
reads as `app` — fail closed, because misreading the other way fires an
irreversible outward-facing action off one tap.

Actions: `open_url` (system browser; needs `params.url`), `open_diff`,
`show_note` (a dialog; needs `params.note`), `start_agent`, and `prompt_agent`
(the agent-performed one; needs `params.prompt`). An `action` a client does not
implement — or a known one missing its required param — must be **dropped, not
...
pane for 6s over a host-wide 5s port scan, with **one** git read per pane (the
same `gitdiff` call `/diff?context=1` answers with); the app refetches on screen
